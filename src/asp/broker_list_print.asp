...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <h1>NSX PARTICIPANT CONTACT SHEET</h1>
		...
		<p><a href="<%= Application("nsx_SiteRootURL") %>">www.nsxa.com.au</a></p>
...
if state <> "" then srch = srch & " AND  (adstate='" & state & "')"
if suburb <> "" then srch = srch & " AND  (adsuburb='" & suburb & "')"

board=ucase(trim(request("board")))
if len(board)<>0 then srch = srch & " AND (addisplayboard LIKE '" & board & "') "

Set ConnPasswords = Server.CreateObject("ADODB.Connection")
Set CmdDD = Server.CreateObject("ADODB.Recordset")
 
ConnPasswords.Open Application("nsx_ReaderConnectionString")
SQL = "SELECT agid,agName,Address,POBOX,AgEmail,Websites,Logo,Strapline,Phone,Fax, tradingauthorisation FROM shagen ORDER BY agName ASC "
...
<% else
		lap = 0
		  ...
		  tradingauthorisation1 = adjtextarea(trim(alldata(10,jj1)))
		  tradingauthorisation2 = adjtextarea(trim(alldata(10,jj2)))
  ...
  web:   <%=web1%><br>
<%
if instr(tradingauthorisation1,"CHESS") > 0 then
	Response.Write "CHESS Securities (T+3): Yes<br>"
end if
if instr(tradingauthorisation1,"CERTIFICATED") > 0 then
	Response.Write "Certificated Securities (T+5): Yes<br>"
end if
%>
  </td>
  <td class=plaintext valign="top">
  ...
  web:   <%=web2%><br>  
<%
if instr(tradingauthorisation2,"CHESS") > 0 then
	Response.Write "CHESS Securities (T+3): Yes<br>"
end if
if instr(tradingauthorisation2,"CERTIFICATED") > 0 then
	Response.Write "Certificated Securities (T+5): Yes<br>"
end if  
%>
  </td>
