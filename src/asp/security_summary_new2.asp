    ...
    <div id="security-header">
        ...
        <div id="security-header-top">
			...
			If Len(coName) > 0 Then
				Response.Write Server.HTMLEncode(coName)
			Else 
				Response.Write "&nbsp;"
			End If
			%></a>
...
<div class="securitylinks" style="padding-top:8px; padding-bottom:8px;">
  <ul>
    <li><a href="/company_details.asp?nsxcode=<%=company_code%>">Company Details</a></li>
    <li><a href="/security_details.asp?nsxcode=<%=security_code%>">Security Details</a></li>
    ...
    <li><a href="/security_capital.asp?nsxcode=<%=company_code%>">Issued Capital</a></li>
    <li><a href="/security_dividends.asp?nsxcode=<%=security_code%>">Dividends</a></li>
	<li><a href="/prices_trades.asp?tradingcode=<%=security_code%>">All Trades</a></li> 
  </ul>
</div>	
...
%>
        <div class="ann-nt">
          ...
          <% If ps = True Then %>
            <img onclick="javascript:ZoomTo('<%=Year(newsDateFrom)%>-<%=FormatDateVal(Month(newsDateFrom))%>-<%=FormatDateVal(Day(newsDateFrom))%>', '<%=Year(newsDateTo)%>-<%=FormatDateVal(Month(newsDateTo))%>-<%=FormatDateVal(Day(newsDateTo))%>')" alt="" src="img/ann_ps1.png" style="float:left;">&nbsp;
          <% End If %>
            ...
            <div class="table-responsive"><table>
                ...
                <tbody>
                 <tr class="sub-header alt">
                    ...
                    <td align="right" width="80">ORDERS</td>
                    <td align="right" width="80">PRICE $</td>
                    <td align="right" width="80">QTY</td>
                    <td align="right" width="80">ORDERS</td>
                    <td align="right" width="250">TRADE DATE</td>
                    <td align="right" width="80">PRICE $</td>
                    <td align="right" width="80">QTY</td>
...
'For rcc = 0 To row_count-2  
'For rcc = 0 To row_count-1  
For rcc = 0 To row_count
%>
<tr <%=row_class%>>
<%
  j=0 
  ...
  For i = 0 To depthPartsCount-1 Step 5
    ...
    If depthType = "buy" Then
      If IsNumeric(depthParts(i+1)) And IsNumeric(depthParts(i+3)) And IsNumeric(depthParts(i+2)) Then
        if rcc = j then
%>
<td width="80" align="right"><%=FormatNumber((depthParts(i+1)/1000),3)%></td>
<td width="80" align="right"><%=FormatNumber(depthParts(i+3),0)%></td>
<td width="80" align="right"><%=depthParts(i+2)%></td>
<%
        Response.Write(VbCrLf)
        printed_td = true
        end if
  ...
  For i = 0 To depthPartsCount-1 Step 5
    ...
    If depthType = "sell" Then
      If IsNumeric(depthParts(i+1)) And IsNumeric(depthParts(i+3)) And IsNumeric(depthParts(i+2)) Then
        if rcc = j then
%>
<td width="80" class="orange" align="right"><%=FormatNumber((depthParts(i+1)/1000),3)%></td>
<td width="80" class="orange" align="right"><%=FormatNumber(depthParts(i+3),0)%></td>
<td width="80" class="orange" align="right"><%=depthParts(i+2)%></td>
<%
        Response.Write(VbCrLf)
        printed_td = true
        end if
...
%>
<td width="250" class="green" align="right" ><%=TradeDateTime%></td>
...
<td width="80" class="green"><%=Status%></td>
</tr>
<%
  Else  
%>
