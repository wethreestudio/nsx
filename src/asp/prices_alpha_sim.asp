...
nsxsearch =  srchregion & srch 
nsxsortorder = "tradingcode ASC"
nsxdb="prices"
nsxboardgroupid="SIM"
nsxboardid=board
'nsxsql = SQL
'nsxsqlflag=true
%>
<!--#INCLUDE FILE="nsx_data_collector.asp"-->
<%
sessionmode=""
...
<%end if%>&nbsp;
	Number of Displayed Securities: <b><%=formatnumber(rc+1,0)%><br>
	</b>Pages:
      <%if currentpage > 1 then %>
                <a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage-1%>&board=<%=board%>&region=<%=displayboard%>">
	<font face="Arial">&lt;&lt;</font></a><a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage-1%>&board=<%=board%>&region=<%=displayboard%>"> Previous <%=maxpagesize%></a> | 
            <%end if%>
      ...
      %>
      <a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=ii%>&board=<%=board%>&region=<%=displayboard%>" ><%=ii%></a> | 
      ...
      %>
             ...
             <a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage+1%>&board=<%=board%>&region=<%=displayboard%>">Next <%=maxpagesize%> 
...
<div align="center"><% if len(Tradingcodes)=0 then %>
		...
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&region=<%=displayboard%>" >All</a> | 
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&traded=1" >Traded Today</a>&nbsp; Sector: <a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&board=ncrp&region=<%=displayboard%>" >Industrial</a> | 
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&board=nprp&region=<%=displayboard%>" >Property</a> | 
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&board=ndbt&region=<%=displayboard%>" >Debt</a> |
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&board=nmin" >Mining &amp; Energy</a> |
		<a href="prices_alpha_sim.asp?nsxcode=<%=tradingcodes%>&currentpage=<%=currentpage%>&board=nrst" >Restricted</a> 
...
<table id="prices_123" cellpadding=2 class="sortable" cellspacing="0" width="100%" bgcolor="#FFFFFF" style="border-bottom:1px solid #808080; ">
    ...
    if rc=-1 then 
       ...
       for jj = st to fh
		...
		if currenteps = "0" or currenteps = ""  then
			pe = 0
		else
			calcprice = prvclose
  ...
  <tr bgcolor="<%=cl(lap)%>" onMouseOver="this.bgColor='#CCCCDD'" onmouseout="this.bgColor='<%=cl(lap)%>'">
    <td class="plaintext" nowrap align=left valign="middle"><font color="<%=col3%>"><a href="prices_alpha_sim.asp?nsxcode=<%=tradingcode%>&coname='<%=issuedescription%>'&region=<%=displayboard%>" title="<%=tradingcode & ": " & issuedescription %>.  Click to see full depth."><%=tradingcode %></a></font></td>
...
<%		end if
	...
	<p class=plaintext><font size="1">page displayed on: <%=cnvddmmyyyy(date) & " " & time%></font>
	</td>
