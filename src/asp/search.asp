...
If t = "issue" Then
	...
	If st="ann" Then
		Response.Redirect "/marketdata/search_by_company?nsxcode=" & tradingcode
		Response.End 	
	End If
...
%>
  <tr<%=c%>> 
      ...
      <td><a href="/summary/<%=UCase(code)%>">Security Code - View summary page.</a> | 
	      <a href="/marketdata/company-directory/announcements/<%=UCase(code)%>/">View Announcements Page</a></td> 
...
%>
  <tr<%=c%>> 
      ...
      <td><a href="/company_details.asp?nsxcode=<%=UCase(code)%>">View Issuer/Company Details</a></td>
      <td><%=Server.HTMLEncode(company)%></td>  
  </tr> 
<%     
    End If   
