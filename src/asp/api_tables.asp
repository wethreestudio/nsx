...
nsxsortorder = SafeSqlParameter(request.querystring("nsxsortorder"))  ' "DESC"  ... sort order
nsxsearch = ""
...
if nsxsqlflag = "TRUE" then
	...
	if len(nsxsearch)<>0 and len(nsxboardid)<>0 then
		nsxsearch = nsxsearch & " AND (pricescurrent.exchid='" & SafeSqlParameter(nsxboardid) & "') AND (issuestatus = 'Active')"
	else
		nsxsearch = " (pricescurrent.exchid='" & SafeSqlParameter(nsxboardid) & "') AND (issuestatus = 'Active')"
	end if 
	...
	if len(nsxsearch)<>0 then SQL = SQL & " WHERE (coAnn.annRelease is not null) AND " & nsxsearch
	if len(nsxsortorder)<>0 then SQL = SQL & " ORDER BY " & nsxsortorder
