...
SQL= SQL & "agDomicile, agFloatDesc, agaccountants, agtrustee, agfacilitators,balancedate,agRegistryId"
...
<tbody> 
  <% if agacn<>"" then%>
  <tr <%=trClass()%>>
    <td align="left" valign="top" width="240"><b>ACN:</b></td>
    <td><%=agacn%></td>
  </tr>
  <%end if%>
  <% if agabn<>"" then%>
  ...
  <%end if%>
  <% 
  sql = "SELECT DISTINCT tradingcode FROM coIssues WHERE nsxcode='" & SafeSqlParameter(id) & "' AND issuestatus='Active' ORDER BY tradingcode DESC"
  ...
  <tr <%=trClass()%>>
	...
	While Not rs.EOF
	%>
		<a href="/summary/<%=ucase(rs("tradingcode"))%>"><%=ucase(rs("tradingcode"))%></a><br>
	<%
		rs.MoveNext 
	Wend 
	rs.Close
  ...
  <tr <%=trClass()%>>
    ...
    <td align="left" valign="top"><%=adjtextarea(agIndustryClass)%></td>
  </tr>
  <%end if%>
  <% if agaddress<>"" then%>
  <tr <%=trClass()%>>
  ...
  <% if agadvisers<>"" then%>
  <tr <%=trClass()%>>
    <td align="left" valign="top"><b>Advisers:</b></td>
  ...
  <% if registryid<>"" then
	Dim registryHTML
	sql = "SELECT r.*, c.CityName, s.[stateb], cc.[Country] FROM registries r JOIN [lookup - cities] c ON r.adCity = c.tid JOIN [lookup - states] s ON r.adState = s.[sid] JOIN [lookup - country] cc ON r.adCountry = cc.cid WHERE r.adid='" & cint(registryid) & "'"
	...
	Else
		i = 0
		If Not rs.EOF Then
			If Len(rs("adName")) > 0 Then registryHTML = registryHTML & rs("adName") & "<br>"
			If Len(rs("adLevel")) > 0 Then registryHTML = registryHTML & rs("adLevel") & "<br>"
			If Len(rs("adBuild")) > 0 Then registryHTML = registryHTML & rs("adBuild") & "<br>"
			If Len(rs("adAddress")) > 0 Then registryHTML = registryHTML & rs("adAddress") & "<br>"
			If Len(rs("CityName")) > 0 AND Trim(rs("CityName")) <> "-" Then registryHTML = registryHTML & rs("CityName") & " " & rs("stateb") & "<br>"
			If Len(rs("stateb")) > 0 Then registryHTML = registryHTML & rs("stateb") & "<br>"
			If Len(rs("Country")) > 0 Then registryHTML = registryHTML & rs("Country") & "<br>"
			registryHTML = registryHTML & "<br>"
			if Len(rs("adPOBOX")) > 0 Then
				registryHTML = registryHTML & "Postal Address<br>"
				Dim poBoxType
				poBoxType = rs("adPOBOXType")
				if Len(poBoxType) = 0 Then poBoxType = "PO Box"
				registryHTML = registryHTML & poBoxType & " " & rs("adPOBOX") & "<br>"
				If Len(rs("adPOSuburb")) > 0 Then registryHTML = registryHTML & rs("adPOSuburb") & "<br>"
				If Len(rs("adPOPcode")) > 0 Then registryHTML = registryHTML & rs("adPOPcode") & "<br>"
				If Len(rs("stateb")) > 0 Then registryHTML = registryHTML & rs("stateb") & "<br>"
			End If
			registryHTML = registryHTML & "<br>"
			if Len(rs("adPhone")) > 0 Then registryHTML = registryHTML & "Phone: " & rs("adPhone") & "<br>"
			if Len(rs("adFax")) > 0 Then registryHTML = registryHTML & "Fax: " & rs("adFax") & "<br>"
			if Len(rs("adEmail0")) > 0 Then registryHTML = registryHTML & "Email: <a href=""mailto:" & rs("adEmail0") & """>" & rs("adEmail0") & "</a><br>"
			if Len(rs("adWeb0")) > 0 Then registryHTML = registryHTML & "Web: <a href=""" & FixHref(rs("adWeb0")) &""" target=""_blank"">" & Replace(rs("adWeb0"),"http://","",1,1) & "</a><br>"
		End If
	End If
	rs.Close
	Set rs = nothing
	...
	If registryHTML<>"" Then
  %>
  <tr <%=trClass()%>>
    <td align="left" valign="top"><b>Share Registry:</b></td>
    <td align="left" valign="top"><%=registryHTML %></td>
  </tr>
  <%
	End If
  End If%>
