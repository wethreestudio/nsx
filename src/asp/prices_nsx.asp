...
if (instr(tradestatus,"SU")>0) and len(tradingcodes)>0 then
    ...
    ELSE
      jj=0
       FOR jj = 0 to rc
		...
		if last <> 0 then 
			marketcap = (CDbl(last) * CLng(currentsharesonissue))/1000000.0
		else
			marketcap = (CDbl(prvclose) * CLng(currentsharesonissue))/1000000.0
		end if
 ...
 select case fmt
 	...
 	txt = txt & high & tb
 	txt = txt & low & tb
 	txt = txt & replace(volume,",","") & tb
 	txt = txt & replace(marketcap,",","") & tb
 	txt = txt & qu & status & qu & tb
 	txt = txt & prvclose & tb
 	txt = txt & replace(currentsharesonissue,",","") & tb
