...
if srt <> "tradingcode" AND srt <> "recorddatestamp" AND srt <> "issuedescription" then srt="symbol"
...
'Response.End
		...
		Else
			volume = 0.0
		End If			
		If IsNumeric(bidqty) Then 
			bidqty = CDbl(bidqty)
		Else
			bidqty = 0.0
		End If			
		If IsNumeric(offerqty) Then 
			offerqty = CDbl(offerqty)
		Else
			offerqty = 0.0
		End If			
		...
		Else
			change = 0.0
		End If			
		If IsNumeric(dchange) Then 
			dchange = CDbl(dchange)
		Else
			dchange = 0.0
		End If			
 ...
 select case fmt
	...
	' Fields
	' "IQN",1.200,"3/10/2013 9:59:53 AM","0","0.00%","iQnovate Limited FPO",0.000,0.000,0.000,0,44.947068,"",1.2,37455890,"01 - Ordinary","",0.000,0,0.900,10000
	' Trading Code,Last Price, Last Trade Date/Time, Price change, Percentage Change, Issue Description, Open, High, Low, Volume, Market Cap, Status, Previous Close, Shares on Issue,Issue Type, Industry Class, Bid, Bid Qty, Offer, Offer Qty
