...
<%
' calculate half year statistics for half year financial report.


cr=vbCRLF
qu=""""
tb=","
startdate = cdate(request("startdate"))
enddate = cdate(request("enddate"))
...
' TODO: Remove IIF functions
SQL = "SELECT DATEPART(Year, TradeDate), SUM(CASE WHEN AddDelete='D' THEN SaleVolume*-1 ELSE SaleVolume END),  SUM(CASE WHEN AddDelete='D' THEN SaleValue*-1 ELSE SaleValue END), Count(PricesTrades.prid), SUM(CASE WHEN AddDelete='D' THEN 1 ELSE 0 END) "
SQL = SQL & " FROM PricesTrades"
SQL = SQL & " WHERE (tradedate>='" & FormatSQLDate(startdate,false) & "') and (tradedate<='" & FormatSQLDate(enddate,false) & "')"
