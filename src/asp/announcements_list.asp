...
Response.Redirect "/marketdata/market_list"
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td width="100%" class="textheader" bgcolor="#FFFFFF" colspan="3" >
      ...
      <h1><b><font face="Arial">REGULATORY NEWS SERVICE</font></b></h1>
    
    </td>
  </tr>
  <tr>
    <td width="100%" class="plaintext" bgcolor="#FFFFFF" colspan="3" height="30">
The NSX Listing Rules requires that listed entities report market significant events to the NSX. This information is important to keep the market informed of their activities.&nbsp;&nbsp;<font size="1">Subscribe to feed: </font>
		<a href="<%= Application("nsx_SiteRootURL") %>/ftp/rss/nsx_rss_announcements.xml">
		<img border="0" src="images/rss/rss.png" width="36" height="14" align="middle" alt=""></a>
...
If Not IsNumeric(currentpage) AND len(currentpage) > 0 Then
	Response.Write ("Page number is not a valid format")
	Response.End
End If
...
if len(displayboard)<>0 then srch = srch & " AND (coissues.displayboard LIKE '%" & SafeSqlParameter(displayboard) & "%') "
...
%>
  <tr>
    ...
    <a href="announcements_list.asp?nsxcode=<%=nsxcodes%>&amp;currentpage=1&amp;group=<%=group%>">First</a> | 
    <b>
    <a href="announcements_list.asp?nsxcode=<%=nsxcodes%>&amp;currentpage=<%=currentpage-1%>&amp;group=<%=group%>">
	<font size="3">&lt;&lt;</font></b><b>
      ...
      %>
      <a href="announcements_list.asp?nsxcode=<%=nsxcodes%>&amp;currentpage=<%=ii%>&amp;group=<%=group%>" ><%=ii%></a> | 
      ...
      <%if maxpages > CurrentPage then %>
             ...
             <a href="announcements_list.asp?nsxcode=<%=nsxcodes%>&amp;currentpage=<%=currentpage+1%>&amp;group=<%=group%>">Next <%=maxpagesize%> 
	<b><font face="Arial" size="3">&gt;&gt;</font></b></a>
	 | <a href="announcements_list.asp?nsxcode=<%=nsxcodes%>&amp;currentpage=<%=maxpages%>&amp;group=<%=group%>">Last</a>
      <%end if%>
    </td>
  </tr>
</table>
...
<table border="0" cellspacing="0" cellpadding="5"  width="100%" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; border-bottom: 1px solid #666666">
  <tr>
    ...
    <td class="plaintext" width="100" bgcolor="#666666"><font color="#FFFFFF"><b>&nbsp;</b></font></td>
    <td class="plaintext" bgcolor="#666666"><font color="#FFFFFF"><b>HEADLINE</b></font></td>
    <td class="plaintext" bgcolor="#666666"><font color="#FFFFFF"><img border="0" src="images/nsxdiag.gif" width="22" height="11" align="right" alt=""><b>DATE</b></font></td>
  </tr>
<%  if WEOF then %>
...
<% else
      	  ...
      	  If IsNumeric(alldata(5,jj)) Then
  ...
  <tr bgcolor="<%=cl(lap)%>" onMouseOver="this.bgColor='#CCCCDD'" onmouseout="this.bgColor='<%=cl(lap)%>'">
    ...
    <td class="plaintext" valign="top"  ><%=name%>&nbsp;</td>
    <td class="plaintext" valign="top"  ><%
   	  ...
   	  'if (Now+daylightsaving)>(Release+0.0139) then 
			...
			response.write "<a href=""ftp/news/" & file & """ title=""" & precise & """ target=""_blank""><b>" & adjtextarea(title) & "</b></a> "
			if dash = 0 then
    ...
    %></td>
     ...
     <td class="plaintext" valign="top" nowrap ><%=fmttf(release)%><img border="0" src="images/nsxdiag25.gif" width="49" height="25" align="right" alt=""></td>
    </tr>
<% NEXT
	end if
	%>
