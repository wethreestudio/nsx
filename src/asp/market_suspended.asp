<%
	Response.Redirect "/marketdata/suspended"
	Response.End
%>
...
<h1>Suspended Securities</h1>
<p>
The following securities are suspended from the NSX Official List. Suspended securities will either delist from NSX or be re admitted to the official list after they have satisfied NSX requirements for listing.&nbsp; When securities are suspended all orders, bids and offers are purged from the market.&nbsp; If suspended companies are under administration please contact their administrator for further information.
</p>
...
SQL = SQL & " WHERE (coIssues.iNewFloat=0) AND (coIssues.Issuestatus ='SUSPENDED')"
...
%>
	...
	<p>There are <b><%=rc+1%></b> suspended securities.</p>
	<p>Page:
      <%if currentpage > 1 then %>
                <a href="announcements_status.asp?nsxcode=<%=nsxcodes%>&currentpage=<%=currentpage-1%>">
	<font face="Arial">&lt;&lt;</font></a><a href="market_suspended.asp?currentpage=<%=currentpage-1%>"> Previous <%=maxpagesize%></a> | 
            <%end if%>
      ...
      %>
      <a href="market_suspended.asp?currentpage=<%=ii%>" ><%=ii%></a> | 
      ...
      <%if maxpages > CurrentPage then %>
             ...
             <a href="market_suspended.asp?currentpage=<%=currentpage+1%>">Next <%=maxpagesize%> 
...
<table cellspacing="0" width="100%" bgcolor="#FFFFFF" cellpadding="5" style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; border-bottom: 1px solid #666666" >
        <tr>
          ...
          <td class="plaintext" align="center" bgcolor="#666666">
            ...
            Profile</b></font></td>
          <td class="plaintext" align="center" bgcolor="#666666">
			<font color="#FFFFFF"><b>Suspended<br>
          Date</b></font></td>
        </tr>
   
        <%
    if WEOF then 
    	response.write "<tr><td colspan=12 class=plaintext>There are no suspended securities.</td</tr>" 
    else
