...
<div class="row subnav-holder"><div class="col-sm-8 breadcrumb-nav">
   <ol class="breadcrumb">
    ...
    <li><a href="/market_eod_nsx.asp">DAILY END OF DAY STATISTICS</a></li>
    </ol></div></div>
</div><!-- /row --> 
</div>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="subpage-center">
		        <h1>DAILY END OF DAY STATISTICS&nbsp;</h1>
                <p>&nbsp;</p>
...
SQL = "SELECT PricesTrades.TradeDate,SUM(CASE WHEN AddDelete='D' THEN SaleVolume*-1 ELSE SaleVolume END), SUM(CASE WHEN AddDelete='D' THEN SaleValue*-1 ELSE SaleValue END), Count(PricesTrades.tradingcode),SUM(CASE WHEN AddDelete='D' THEN 1 ELSE 0 END)"
...
%>
	...
	<a href="market_eod_nsx.asp?currentpage=<%=currentpage-1%>"> &lt;&lt; Previous <%=maxpagesize%></a> | 
            <%end if%>
      ...
      %>
      <a href="market_eod_nsx.asp?currentpage=<%=ii%>" ><%=ii%></a> | 
      ...
      <%if maxpages > CurrentPage then %>
             ...
             <a href="market_eod_nsx.asp?currentpage=<%=currentpage+1%>">Next <%=maxpagesize%> &gt;&gt;</a>
      <%end if%>
...
<thead>
   ...
   cl = array("odd","even")
    ...
    %>
  <tr class="<%=cl(lap)%>">  
    <td  align=right ><%=Day(tradedate) & "-" & MonthName(Month(tradedate),true) & "-" & year(tradedate)%></td>
