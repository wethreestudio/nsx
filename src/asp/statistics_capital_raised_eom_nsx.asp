...
<div class="row subnav-holder"><div class="col-sm-8 breadcrumb-nav">
   <ol class="breadcrumb">
    ...
    <li><a href="/statistics_capital_raised_eom_nsx.asp">Capital Raised By Month</a></li>
    </ol></div></div>
</div><!-- /row --> 
</div>
...
SQL = ""
SQL = SQL & "SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, [capitalexdate]), 0), Sum((CASE WHEN [capitalconsideration] IS NULL THEN 0.0 ELSE [capitalconsideration] END)) AS [capitalconsideration], sum((CASE WHEN [capitalchange] IS NULL THEN 0.0 ELSE [capitalchange] END)) AS [capitalchange] "
SQL = SQL & "FROM [capital] "
SQL = SQL & " WHERE (((Capital.CapitalType) Not Like '%option%' AND (Capital.CapitalType Not Like '%escrow%')) AND CapitalChange IS NOT NULL AND ((Capital.CapitalChange)>0)) "
SQL = SQL & "GROUP BY DATEADD(MONTH, DATEDIFF(MONTH, 0, [capitalexdate]), 0) ORDER BY DATEADD(MONTH, DATEDIFF(MONTH, 0, [capitalexdate]), 0) DESC "
'Response.Write SQL
...
cl = array(" class=""odd"""," class=""even""")
    ...
    %>
<tr<%=cl(lap)%>> 
    <td align=right ><%=MonthName(Month(capitaldate),true) & "-" & year(capitaldate)%></td>
