...
<h1>Share Registry List by Issuer</h1>
<p>Below is a list of Share Registry providers and a list of securities that 
	these registries represent. Under the rules all Issuers are required to have 
	an acceptable share registry service to effect electronic settlement.</p>
...
SQL = "SELECT DISTINCT coDetails.nsxcode, coDetails.coName, coDetails.agregistry, coIssues.IssueStatus "
SQL = SQL & " FROM coDetails INNER JOIN coIssues ON coDetails.nsxcode = coIssues.nsxcode"
SQL = SQL & " WHERE (((coIssues.IssueStatus)='active' or (coIssues.IssueStatus)='suspended'))"
SQL = SQL & " ORDER BY coDetails.agregistry"
...
%>
  ...
  <tr>
    <td width="100%" class="plaintext"><b>Issuers: <%=rc+1%>&nbsp;-&nbsp;Page:&nbsp;</b><%if currentpage > 1 then %>
    
                <a href="registry_list_by_security.asp?region=<%=board%>&currentpage=<%=currentpage-1%>">
	<font face="Arial">&lt;&lt;</font></a><a href="registry_list_by_security.asp?region=<%=board%>&currentpage=<%=currentpage-1%>"> Previous <%=maxpagesize%></a> | 
            <%end if%>
      ...
      %>
      <a href="registry_list_by_security.asp?region=<%=board%>&currentpage=<%=ii%>" ><%=ii%></a> | 
      ...
      <%if maxpages > CurrentPage then %>
             ...
             <a href="registry_list_by_security.asp?region=<%=board%>&currentpage=<%=currentpage+1%>">Next <%=maxpagesize%> 
